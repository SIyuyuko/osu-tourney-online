<template>
  <div class="flex flex-row gap-4">
    ...
    <div class="flex flex-row gap-4">
      <label v-for="option in options" :key="option" class="flex items-center gap-1 cursor-pointer">
        <div class="relative flex items-center justify-center w-4 h-4">
          <div
            class="absolute w-4 h-4 border border-[#13c2c2] rounded-full transition-all"
            :class="{ 'border-[#13c2c2]': modelValue === option }"
          ></div>
          ...
          <input
            ...
            @change="handleValueChange(option)"
    ...
    <div class="flex flex-row h-[1.8rem] w-32">
      ...
      />
      <span
        class="w-16 border border-[#d9d9d9] dark:border-[#424242] border-l-0 rounded-r-[0.6rem] bg-[#fafafa] dark:bg-[#1d1d1d] flex justify-center items-center text-gray-700 dark:text-gray-200"
      >
        {{ $t('command.sec') }}
      </span>
    </div>
